...
cd binkd
sh binkd.Slack64Build
cd ../husky
sh husky.Slack64Build
cd ../golded
sh golded.Slack64Build+


echo ''
echo '------------------------------------------------------------------------------'
echo "   Done! Creation of packages BinkD, Husky HPT and GoldEd+ are finished."
echo '   Install *.txz packages from /tmp directory: '
echo
echo  "installpkg `ls /tmp/binkd-1*86_64*.txz | head -n1`"
echo  "installpkg `ls /tmp/husky*86_64*.txz | head -n1`"
echo  "installpkg `ls /tmp/golded-*86_64*.txz | head -n1`"
