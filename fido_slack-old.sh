...
cd binkd
sh binkd.SlackBuild.old
cd ../husky
sh husky.SlackBuild.old
cd ../golded
sh golded.SlackBuild+.old
...
echo
echo  "installpkg `ls /tmp/binkd-1*i486*.tgz | head -n1`"
echo  "installpkg `ls /tmp/husky*i486*.tgz | head -n1`"
echo  "installpkg `ls /tmp/golded-*i486*.tgz | head -n1`"
