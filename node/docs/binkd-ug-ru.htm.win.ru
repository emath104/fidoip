...
<HEAD>
	...
	<META NAME="CHANGEDBY" CONTENT="Станислав Дёгтев">
	<META NAME="CHANGED" CONTENT="20050514;12133378">
	<STYLE>
	<!--
...
сетям.</P>
<P>В представленном документе описан <FONT FACE="Courier New, monospace">binkd</FONT>
версии 0.9.9 (версия с исправлениями после релиза 0.9.8, выпущенного в ноябре 2004 года).</P>
...
<UL>
	...
	(BSO), Long BSO (LBSO) либо Amiga Style Outbound (ASO). (И для
...
использовать преимущества этого транспорта TCP/IP. В дальнейшем к
...
0.9.8, 0.9.9, являющиеся исправленными релизами после 0.9.5.
Следующая ветка обозначена как 1.0 и версии в ней нумеруются так:
1.0a-номер_исправления, например binkd 1.0a-295. По окончании
разработки версии 1.0 будет выпущен релиз 1.0 и от него будет вестись
ветка 1.0-stable, в случае обнаружения и исправления ошибок будут
выпущены исправленные версии 1.0.1, 1.0.2 и далее. После выпуска
релиза 1.0 начнётся разработка версии 1.1 по такому же сценарию, как
и 1.0.</P>
...
<P>Напоминаю, что речь идёт о <FONT FACE="Courier New, monospace">binkd</FONT>
версии 0.9.9 (точнее &ndash; о ветке 0.9.5-stable, то есть всё
написанное действительно и для версий 0.9.5, 0.9.6, 0.9.7, 0.9.8, также
будет действительно для версий 0.9.10, 0.9.11 и т.д. - если они будут
выпущены). Для находящихся в разработке версий 1.x это руководство
применимо с поправками.</P>
...
<OL>
	...
	0.9.5-stable из одного из официальных источников (либо из источника,
...
<OL>
	...
	версией binkd ветки 0.9.5-stable для вашей ОС из одного из
...
команд ($ - приглашение командного процессора ОС, за ним  через
пробел указана команда, которую нужно ввести, в скобках описаны
действия скрипта и сделаны поясления):</P>
...
c:\\bbs\\outbound\\musicnet 333<BR>domain fido alias-for
...
для 32-хбитных версий <FONT FACE="Times New Roman, serif">Windows</FONT>,
кроме &laquo;!&raquo;, можно использовать модификаторы &laquo;@&raquo;
и &laquo;@@&raquo;. &laquo;@&raquo; означает запуск програмы в
отдельном окне консоли, &laquo;@@&raquo; - в скрытом окне.</FONT></P>
...
имя принятого файла с путём.  В пятом примере скрипт будет выполнен
по окончании сеанса связи с узлом, в строке <FONT FACE="Courier New, monospace">node</FONT>
...
<UL>
	...
	для <FONT FACE="Courier New, monospace">binkd</FONT> адрибуты
...
1996-2003 Dima Maloff and others.<BR><BR>This program is free
...
указывается параметром опции вплотную или чеерз пробел. В командной
...
и <FONT FACE="Courier New, monospace">binkd/w9x</FONT>
и в других вариантах <FONT FACE="Courier New, monospace">binkd</FONT>
не воспринимается.</P>
<P>Примеры:</P>
...
<H2>5.16. -v (строчная буква &laquo;v&raquo;).</H2>
<P><FONT FACE="Courier New, monospace">Binkd</FONT>, вызванный с
...
например:</P>
<P><FONT FACE="Courier New, monospace">Binkd 0.9.8 (Nov 29 2004
13:47:40/Win9x)<BR>Press any key...</FONT></P>
			...
			<THEAD>
				<TR>
					<TD WIDTH=782 VALIGN=TOP>
						<P><FONT FACE="Courier New, monospace">#!/bin/sh</FONT></P>
						<P><FONT FACE="Courier New, monospace">case $1 in</FONT></P>
						<P><FONT FACE="Courier New, monospace">start)<BR>su fido -c
						&laquo;/usr/local/sbin/binkd -Dq
						/usr/local/etc/fido/binkd.conf&raquo;<BR>;;</FONT></P>
						<P><FONT FACE="Courier New, monospace">stop)<BR>kill `cat
						/var/run/binkd.pid`<BR>;;</FONT></P>
...
style outbound&raquo;, этот фотмат очереди получил своё название от
...
по BinkleyTerm.</P>
<P>BSO готовится к стандартизации в FTSC и опубликован в спецификации FSP-1034</P>
...
<H3>7.1.2. ASO.</H3>
<H3>7.1.3. Файлбоксы, совместимые с мейлером &laquo;T-Mail&raquo;.</H3>
...
<H3>7.1.4. Файлбоксы, совместимые с мейлером &laquo;The Brake!&raquo;.</H3>
...
<HR>
$Id: binkd-ug-ru.htm,v 1.1.2.7 2007/04/17 18:24:27 stas Exp $
</BODY>
</HTML>
